...
1. 本地把源码 clone 下来（浅克隆，只要一个版本）：
   ```bash
   git clone --depth 1 --branch v2026.6.4 \
     https://github.com/vaultwarden/vw_web_builds.git /tmp/bw
   ```
2. 在 `/tmp/bw` 里直接改源码（改完能在本地 `npm ci && cd apps/web && npm run dist:oss:selfhost` 验证就更稳）。
3. 生成补丁（**只包含你改的文件**）：
   ```bash
   cd /tmp/bw
   git diff -- <你改过的路径...> > /path/to/warden-worker/webvault/patches/01-描述.patch
   ```
   注意用 `git diff` 而不是 `git diff HEAD`，也**不要**用 `git format-patch`
   （我们只需要内容差异，不需要提交元信息）。
4. 本地先试打一遍，确认能干净应用：
   ```bash
   git apply --check -3 /path/to/.../01-描述.patch
   ```
