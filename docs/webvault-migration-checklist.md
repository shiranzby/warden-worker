...
> ### ✅ 上线记录 · 第十七批 / U 段（移动端第五轮反馈 4 条，全部落地，2026-09-14）
> 版本号 `2026.8.6 → 2026.8.7`。依旧**不是 L4 迁入**，来自用户第五轮移动端反馈。
> 本轮 4 条里，**两条是"症状与根因差得很远"**（问题2 的"没有保存按钮"其实是页脚被自己的底部标签栏盖住；
> 问题3 的"选项列没作用"其实一直是可用的**逐行**菜单），**一条是策略级重做**（下拉定位方向），
> **一条是全站交互统一**（按下展开 → 松手展开）。
>
> | 需求（用户原话要点） | 根因 | 落法 |
> |---|---|---|
> | **问题1**「安全页顶部与下面的会话超时/主密码等**颜色差距过大**；希望那一行背景保持全白，但选中的蓝色和下划线保留」 | 共享组件 `header.component.html` 里写的是 `has-[[data-tabs]:not(:empty)]:tw-bg-background-alt` —— **只要页头里出现 tab 条**（安全页/订阅页通过 `slot="tabs"` 放了 `bit-tab-nav-bar`），整条页头（标题区 + tab 条）就铺上 `--color-background-alt`（实测 `rgb(247,249,250)`），而内容区是纯白 ⇒ 视觉上"断层" | 窄屏把页头**背景改透明**（不是删规则）：透明后露出 `main#main-content` 的白底，浅色/深色主题都自动正确（不写死 `#fff`）。选中项的蓝字与 `::after` 下划线在 `TabListItemDirective` 里，**与本条无关**，原样保留（实测 `rgb(18,82,163)` / opacity=1） |
> | **问题2a**「删除日期 / 谁可以查看点开后的选项**位置并不正常、是悬浮出来的**」 | `applyGeometry()` 的方向判据是 `const up = above > below` —— **只比两边可用空间**。两个下拉在对话框里上下空间几乎相当（下方 351 / 上方 367，差 16px）⇒ 被判成"向上"，7 个选项的面板被摆到 **y=8px**（屏幕顶端），看着跟自己的输入框毫不相干 | 方向改成"**能放下就向下**"；并且**先量面板的实际高度再定位**（选项少时面板真实高度 277px ≪ 可用空间，按可用空间反推会把面板推飞）。向下 `top = r.bottom + GAP`，向上 `top = r.top - GAP - height` |
> | **问题2b**「这两个页面的顶部都分别有『新增文本 Send』『新增文件 Send』，**占用高度过高**」 | 对话框页头 `bit-dialog > section > header` 在窄屏带 `tw-px-6 tw-py-4`（上下各 16px） | 窄屏把上下内边距压到 **2px**（标题区高度 45px） |
> | **问题2c**「这两个新增页**底下也没有新增或者保存的按钮**什么的，似乎没用实质性的作用」 | 🔴 **按钮一直在**，是**被底部标签栏盖住了**：实测页脚「保存/取消」在 `[783..823]`，而固定底栏 `#warden-tabbar` 占 `[781..844]` —— **完全重叠** ⇒ 用户"看不见 + 点不到"，于是以为这页没有提交入口 | `body:has(bit-dialog) #warden-tabbar { display:none }`（**开对话框时藏掉标签栏**）。选这条而不是"给对话框加 padding 把页脚顶上去"：后者标签栏仍可点，用户能在模态开着时切走；模态本就该独占屏幕。**顺带把所有对话框的页脚遮挡问题一起修好**。副作用是好的：`tabbarInset()` 自动读 0，下拉可用高度变大 |
> | **问题2d**「要分享的文本，我希望像自定义域名一样**压缩高度为一行**即可，私密备注也如此」 | 两个字段是 `rows="3"` 的 textarea | 窄屏压到 **32px**（= K3 的 input 高），`line-height` 同步 32px 让单行文字落在垂直中点。⚠️ 选择器**必须用 `formcontrolname` 钉死** `text`/`notes`，否则会把条目备注、指定人员那种"该保持多行"的框一起压掉（第十六批 S2 踩过同一个坑） |
> | **问题2e**「最大访问次数最右边可以给个 **− + ** 的按钮，默认 placeholder 不填为**不限制**」 | 原本既没有步进按钮，空值也没有任何提示 | 加 `bitSuffix` 槽位的 ⊖/⊕ 两个按钮 + `[placeholder]="maxAccessCountPlaceholder"`。**让 − 成为 + 的逆运算**：`… ↔ 3 ↔ 2 ↔ 1 ↔ 空(=不限制)`，1 再减一次回到"不限制"而不是卡在 1（用户特别强调"默认不填为不限制"）。空的控件用 `""` 而非 `null`，与原有 `valueChanges` 约定一致 |
> | **问题2f**「**缺失了密码的设置**，应该加上是否启用密码，启用了就可以额外加密码，密码可以有眼睛、随机的按钮、复制的按钮」 | 🔴 **误会：密码 UI 一直都在**（`send-details.component.html` 里，`authType === Password` 时渲染眼睛/生成/复制三个按钮）。它"看不见"是因为**用户根本选不到那个选项** —— 「谁可以查看」的下拉一展开就飘到屏幕顶上（问题2a），够不着 | **无需新增**。修好 2a 之后实测：选「拥有您设置的密码的任何人的」⇒ 密码框出现，`btnCount=3`（眼睛/随机/复制齐全），框高 32px。「文件 Send」同样对待（与文本 Send 共用同一套 options/details 组件） |
> | **问题3**「发送页切换条（所有 Send / 文本 Send / 文件 Send）**上方有空白冗余**；另外**选项这列似乎没有实质功能**，还是说要新增了才有的？」 | 投影容器写的是 `<div class="tw-mt-4" slot="filters">`，而它上面已经没有可见内容（页头被 `warden-bare-header` 藏掉）⇒ 这 16px 就是用户看到的"冗余" | `main#main-content tools-send-list > [slot="filters"] { margin-top: 0 }`（仅窄屏）。**并在回复里答复用户的问题**：表中「选项」列是**逐行**的 `…` 菜单（复制 Send 链接 / 移除密码 / 删除），**列表为空时它当然什么也不显示** —— 它是每行一个的操作入口，不是"没功能的全局控件" |
> | **问题4a**「现在摸到了、刚点到就触发，我希望是**松手的时候**触发」 | 展开时机挂在 `pointerdown` 上 | 拆成两个事件：`onFieldPointerdown` **只 `preventDefault()`**（取消它后浏览器不再补发兼容 mouse 事件，否则 ng-select 自己的 `handleMousedown()` 会二次 toggle ⇒ 表现是"开了立刻又关"）；`onFieldPointerup` 才 `toggle()`；再用 `pressStartedOnField` 记"这一下起手在控件本体上"（否则"按住往外拖"松手也会弹面板） |
> | **问题4b**「工具页的导出页，文件格式选择框点击后是**单独弹出个弹窗**，希望和其它部分一样在框下面弹出选项」 | 导出页那个是**原生** `<select bitInput>` —— 手机上点它会弹**系统自带的全屏选择器**，与站内其它下拉完全不是一个观感 | 换成 `bit-select` + `bit-option`，走全站同一套面板（窄屏松手展开、点空白/再点框收回）。⚠️ 这条**没有 grep 判据**（Angular 把模板编译成 JS，原生 select 与 bit-select 的差别在产物里没有稳定字面量），改用**运行期探针**守，见下 |
>
> | 步骤 | run | 结果 | 耗时 | 备注 |
> |---|---|---|---|---|
> | `Build Web Vault (patched)`（`version=v2026.8.7`） | **`34866333630`** | ✅ success | **5m16s** | **16/16 步**；断言步骤 **25 组全绿**（本批新增第 25 组 U 段） |
> | `push-cloudflare` dispatch（workflow 名 `Build`） | **`34867003294`** | ✅ success | **4m36s** | **18/18 步**；第 8 步 `Verify frontend is our patched build (fail hard)` 绿 |
>
> - 产物：`bw_web_vault-v2026.8.7`（artifact id `10356708791`，**35.8 MB**）。
> - 前端源码：fork `shypwd` = **`75de66fbd7246a6ad09e7a0ea7c9bfac67cd9490`**（U 段功能提交，10 files / +282 −23）；
>   warden-worker `main`：两条 run 的 `head_sha` 均为 **`ac80fe6`**（CI 第 25 组 + 四处版本默认值 + 白名单收录两个探针），
>   该次 push 带 `[skip ci]`（已核 run 列表无新增自动部署）。
> - **线上取证**（走 `-x http://127.0.0.1:7890`）：`vw-version.json` = **`{"version":"2026.8.7"}`**；
>   主 JS `app/main.` **`05a80cbf7e5dc88d0603` → `3e55c64b7653e66ef480`**（`5 484 139 B`，上版 `5 482 423 B`）——
>   **asset 哈希变了 = 部署生效最硬的证据**；线上 `/css/vaultwarden.css` **101 511 B**（上版 95 557 B）
>   与 fork 源码**逐字节 `cmp` 一致**（原样拷贝、不压缩）。
> - **产物级标记**（线上 `main.js`，9 个字面量全部命中）：`pressStartedOnField` 4 / `onFieldPointerup` 2 /
>   `stepMaxAccessCount` 3 / `maxAccessCountPlaceholder` 2 / `max-access-count-increase` 1 /
>   `max-access-count-decrease` 1 / `wardenUnlimited` 1 / `wardenIncrease` 1 / `wardenDecrease` 1。
>   线上 `vaultwarden.css`：段横幅 + U1/U2/U3/U4/U5 五条选择器各命中（`bit-dialog > section > header` 2 次）。
> - **线上运行时 48/48 PASS**（`probe-u17-live.mjs`，真站 + **显式代理** + 390×844 + 触摸）：
>   ① 版本 2026.8.7 且能取到主 JS；②~⑩ 9 个 main.js 标记 + 6 个 css 标记；
>   ⑪ 导出页 `native select` **0** 个、`bit-select` 1 个；⑫ 点它弹的是**站内面板(4 个选项)**、⑬ 面板贴框下沿
>   (panel top 157 / field bottom 149)、⑭ 再点收回；⑮ 安全页页头背景 `rgba(0,0,0,0)`、⑯ 选中 tab 仍 `rgb(18,82,163)`、
>   ⑰ 下划线 opacity=1；⑱ 发送页 `mt=0px`、⑲ 切换条 y=6；⑳ 对话框页头 `padTop=2px`/高 45、㉑ 标签栏 `display:none`、
>   ㉒ 页脚「保存/取消」都在且 `bottom=823 ≤ 844`（不再被盖）、㉓㉔ 两个文本框均 **32px**、㉕ placeholder「不限制」、
>   ㉖ ⊖/⊕ 两个按钮都在、㉗㉘ 两个下拉面板**都贴框下沿**(373/365 与 464/456)、㉙㉚ 再点都收回、
>   ㉛ **按下不展开(down=0)** / ㉜ **松手才展开(up=1)**、㉝ 密码框出现且 **3 个按钮齐全**、㉞ **0 pageerror**。
> - **线上视觉取证**（`.scratch-u17-shots.mjs` → `shots/u17/live-0{1..8}-*.png`，只拍版式、不含保险库条目内容）：
>   安全页 tab 条与内容同白、导出页下拉**贴框展开**、发送页切换条贴顶、
>   对话框**紧凑页头 + 一行的文本框 + 「不限制」+ ⊖/⊕ + 底部「保存/取消」**。
> - **本机复验**：窄屏 390×844 `probe-u17-verify.mjs` **35/35**；桌面 1280×860 `probe-u17-desktop.mjs` **12/12**
>   （证明 U1~U5 与"松手展开"**只在窄屏生效**，桌面端 tab 条灰底、对话框宽松页头、多行文本框、16px 间距一个没动）。
> - 2 步 dispatch（只留 `--retry 3`）⇒ 各**只触发 1 个 run**；推 `main` 那次带 `[skip ci]`（已核 run 列表无新增自动部署）。
>
> #### 🔴 本批四条必须记住的教训
>
> 1. **用户描述的"缺功能"，先验证"那个功能是不是被自己人挡住了"。** 问题2c 说"底下没有保存按钮"、
>    问题2f 说"缺失密码设置" —— 两者都是**误判**：保存/取消一直在（被底部标签栏压住，实测重叠
>    `[783..823]` vs `[781..844]`），密码三件套也一直在（只是那一步够不着）。
>    ⇒ 症状出现时先量**几何重叠**，别急着加按钮。
> 2. **"浮层不随路由卸载"会把上游所有页面测量污染成退化值。** cdk overlay（对话框）挂在 `body`，
>    开着时应用根 `main#main-content` 被压成 **20px 宽** ⇒ 之后量的任何东西都是 `11,15,0x34` 这种
>    退化盒。这个假象与"功能没生效"**一模一样** —— 导出页那条我一度判成产品缺陷。
>    ⇒ **探针设计上就让"量浮层之外的页面"排到打开浮层之前**，不要事后清场（Escape 关不掉、
>    点取消不稳、`reload()` 会被踢去 `#/lock?promptBiometric=true`、`goto` 只改 hash 属
>    same-document 导航根本不重载）。
> 3. **`preventDefault()` + `pointerup` 是"松手才展开"的正确拆法，但别在它前面按 Escape。**
>    实测同一控件 5 种变体：`touchStart → touchEnd([])` 单独跑是 `down=0 / up=1`（正确）；
>    一旦在它**前面加一次 Escape**，就变成 `0/0`，而且**之后的普通 tap 也开不了**（控件像卡住）。
>    本地那段前面恰好没有 Escape 所以一直绿 —— 写探针时别"顺手对齐"成有 Escape 的版本。
> 4. **取线上 asset 哈希要**连取两次**并确认稳定。** 本批第一次取到 `main.315b37140f687b534696.js`，
>    稍后再取就变成 `main.3e55c64b7653e66ef480.js` 并从此稳定 —— 前者是**边缘缓存尚未收敛**时的旧值。
>    只取一次就记进文档，会把 T 段的哈希当成本批的哈希，日后核对时全是矛盾。
>    （另：首页引用主 JS 用的是**相对路径无前导斜杠**的 `app/main.<hash>.js`，
>    正则里写死 `/app/main...` 会匹配不到 ⇒ 断言静默变空，看着像"产物没上线"。）
>
> **回滚**：`git revert` fork 的 U 段提交（`select.component.{ts,html}` + `send-options.component.{html,ts}` +
> `export.component.html` + `vaultwarden.css` + `locales/{en,zh_CN}/messages.json` + 四处版本里的两处）
> + warden-worker（CI 第 25 组 + 两处 workflow 版本默认值）并把 `BW_WEB_VERSION` 改回 `v2026.8.6`
> （+ 同步 lock 与 `inputs.version` 默认值），再重跑 `Build`。
...
> ### ✅ 上线记录 · 第十九批 / W 段（用户验收第十八批后提的 2 条，全部落地，2026-09-15）
> 版本号 `2026.8.8 → 2026.8.9`。**纯 CSS 改动**，没有动任何 `.ts` / `.html`。
> 这两条的来历比改动本身更值得记：**都是第十八批"判据取错了对象"漏到用户手里的**。
>
> | 需求（用户原话要点） | 根因 | 落法 |
> |---|---|---|
> | ① 「要分享的文本」输入框还是过高，要和 Send 名称一样高；附加选项的私密备注、新增文件的私密备注也要一样 | 输入框**本体**早就统一成 38px（第十八批断言过，全绿）；高的是**带边框的可见框**：外层组件自带 `has-[textarea]:!tw-py-3` 上下各 12px ⇒ 名称框 40 / 文本框 64 | `bit-dialog:has(tools-send-form) bit-form-field div:has(> textarea[bitInput]) { padding-top/bottom: 0 }`。收窄到 `:has(tools-send-form)` 是必须的：条目编辑对话框的多行备注**故意**让它高 |
> | ② 发送页右上角的「新增 Send」高度没有与左边切换一致，文字大小不一致，也不居中 | 切换条 `bit-toggle-group` 可见高度 42（内含 32 的 pill，垂直居中在 y=57）；新增按钮 32 且**顶对齐**（y=52，bottom 84 vs 94）；字号 12.5px vs 切换标签的 14px | 让按钮撑满宿主 42px + flex 居中 + 字号对齐 14px（加锚点 `--warden-filters-new-fs: 14px`） |
>
> **🔴 本次最贵的一条教训（权重）**：W1 第一版写成 `div:has(> textarea)` 时，
> **本地验收全绿、实际完全没生效**。原因是对方那条是 Tailwind 的 `has-[textarea]:!tw-py-3`
> —— 一个**类** + `!important`；而 `…bit-form-field div:has(> textarea)` 里
> `:has(> textarea)` 只贡献 (0,0,1)，整条 (0,0,5)。
> **两条都带 `!important` 时比的是权重 (a,b,c)，与书写先后无关**（MEMORY ① 早就写过，
> 这次是第一次在"我新写的规则被压掉"这个方向上撞上）。
> 把 `[bitInput]` 写进 `:has()` ⇒ 该部分权重变 (0,1,1) ⇒ 整条 (0,1,5) > (0,1,0) 才生效。
> 判据：用 CSSOM `insertRule` 逐条试过 —— 不带属性 62px（无效），带属性 38px（生效）。
>
> **🔴 第二条教训（判据对象）**：第十八批断言的是"输入框**本体**高度 38px"，
> 用户看到的却是"**带边框的可见框**"。本体早就对了，所以断言全绿而问题仍然存在。
> 本批的探针改为量"第一个有边框的祖先"（`.box`），并在脚本头部把这条写进注释。
> ⇒ **判据要对着用户看见的东西量，不要对着内部实现量。**
>
> | workflow | run | 结论 | 耗时 | 备注 |
> |---|---|---|---|---|
> | `Build Web Vault (patched)`（`version=v2026.8.9`） | **`34893054366`** | ✅ success | **5m43s** | **27 组断言全绿**（本批新增第 27 组 W 段） |
> | `Build`（部署） | **`34893696036`** | ✅ success | **4m18s** | 线上 `/vw-version.json` = `2026.8.9`，`main.0b3fb6d186f08fbb196d.js` |
>
> - 前端源码：fork `shypwd` = **`27407ba831`**（3 files，`vaultwarden.css` 为主）；
> - 本仓库：**`f2b4a75`**（第 27 组断言 + 版本四处 8.8→8.9 + 守卫脚本加 W 段 + 交付文档 + README）。
> - 离线证伪：`.deploycheck/probe-s15-guard.py` 跑通 —— 待证伪 23 条（R6/S3/T2/U5/V5/W2），
>   22 条"提成顶格"全部被抓住，1 条（`.warden-domain-row`）因"只出现在更长选择器里、从不在行首"跳过；
>   对照组 16 条全部不红。
> - 运行期验收：`probe-w19-verify.mjs` 本地 16/16、线上 **16/16**。
>   线上实测：名称/文本/备注 可见框均 **40px**（改前文本与备注 64px）；
>   新增 Send 与切换条 **42 = 42**、上下沿同 y、字号同为 **14px**、文字 cy 73 = 73。
>
> **回滚**：`git revert` fork 的 W 段提交（`vaultwarden.css`）+ 本仓库（第 27 组 + 四处版本），
> 把 `BW_WEB_VERSION` 改回 `v2026.8.8`（+ 同步 lock 与 `inputs.version`），再重跑 `Build`。
> 注：W2 与 V3b 是同一选择器，回滚 W2 会自动回到 V3b 的 32px —— 这正是"每批改动留在自己段里"的好处。

---

> ### ✅ 上线记录 · 第二十批 / X 段（**电脑端**一批反馈 5 条，落地 4 条，2026-09-15）
> 版本号 `2026.8.9 → 2026.8.10`。前十九批全部是移动端反馈，**这一批第一次正对电脑端**。
> 第 5 条（设置页排版）只出渲染稿、未改源码，故不在本批。
>
> | 需求（用户原话要点） | 根因 | 落法 |
> |---|---|---|
> | ③ **所有带选项的表单点了没反应**，最右侧还会多出一条滚动条 | **不是样式问题**：`onOpen()` → `fitPanelToKeyboard()` 的**桌面早退分支里调了 `clearGeometry()`**，把 ng-select 刚写好的 `position/top/left/width` 全清掉 ⇒ 面板掉回"文档末尾的静态位置"（视口下方一屏、整屏宽），同时把文档撑高 | 早退分支只 `stopGeometryWatch()`；"交还几何"挪到 `onNarrowChange()`（只在**变成宽屏那一刻**） |
> | ① 右上角那个元素没用（点了就回首页）；右上角头像与设置里的不一致 | 宫格 = `product-switcher`（窄屏早在 I/R 段就藏了，桌面端当初**刻意保留**）；页头头像用的是 Bitwarden 自带 `bit-avatar`（读服务端首字母），而账户卡片用的是我们本地选的图（`--warden-avatar`） | X1 全局隐藏 `product-switcher`；X1b 让页头 `bit-avatar svg` 也铺 `var(--warden-avatar)` 并藏掉首字母 |
> | ② 要分享的文本 / 私密备注在电脑端也要"真正的一行" | U4（textarea 38px）与 W1（清外层上下各 12px 内边距）此前都**只在窄屏** | X2：两条**同时**提升为全局（只改一条可见框仍是 64px），规则搬到文件末尾 X 段，U4 处留指针注释 |
> | ④ 最大访问次数输入框多出一组原生上下箭头 | 它右侧已有应用自带的「−/+」，而它是 `type="number"` | X4：`appearance: textfield` + 清 `::-webkit-inner/outer-spin-button`，范围限发送表单 |
>
> **🔴 本批最重的一条教训：验收只跑一个视口宽度，让电脑端功能性缺陷活了 6 批。**
> `clearGeometry` 是**第十三批（Q 段 `3f26c76552`）**加进去的，第十三~十九批的验收脚本
> 一律写死 `viewport: { width: 390, height: 844 }` —— 电脑端**从未被验证过**。
> ⇒ 新增 `probe-x20-verify.mjs`：用 `RECON_W/RECON_H` 一个脚本跑两档，**默认值刻意是桌面 1280**。
>
> **另一条：CI 的负向守卫在这次改动里红了两次，而两次它都是对的** —— U4/W1/R 段那条
> `product-switcher` 被提升为全局，守卫正是在拦"把窄屏规则误提成全局"。
> 处置：把字面量加进旧批次的 `case` 跳过列表（**存在性断言保留**），说明注释写在 `case` **之前**
> （写进 case 体里会被证伪脚本当成模式）。
>
> | workflow | run | 结论 | 耗时 | 备注 |
> |---|---|---|---|---|
> | `Build Web Vault (patched)`（`version=v2026.8.10`） | **`34955593956`** | ✅ success | **5m11s** | **28 组断言全绿**（本批新增第 28 组 X 段） |
> | `Build`（部署） | **`34956126940`** | ✅ success | **4m50s** | 线上 `vw-version.json` = `2026.8.10`，`main.0b589ad14447126231a2.js` |
>
> - 前端源码：fork `shypwd` = **`8f16fcee53`**（4 files）；本仓库 = **`50685a0`**。
> - 离线证伪：待证伪 20 条全部被抓住，对照组 19 条不红；X 段被正确识别为"**整批没有守卫**"
>   （它是第一批"整批规则本来就该全局、故意不加守卫"的改动）。
> - 运行期验收：`probe-x20-verify.mjs` 桌面 **21/0**、移动 **21/0**；`probe-w19-verify.mjs` 16/0。
>
> **回滚**：`git revert` fork 的 X 段提交（`vaultwarden.css` + `select.component.ts`）+ 本仓库
> （第 28 组 + 四处版本 + 三处守卫放开），把 `BW_WEB_VERSION` 改回 `v2026.8.9`，再重跑 `Build`。

---

> ### ✅ 上线记录 · 第二十一批 / Y 段（电脑端下拉"关不掉"，2026-09-15）
> 版本号 `2026.8.10 → 2026.8.11`。**只有 `.ts`，没有一行 CSS。**
>
> | 需求（用户原话要点） | 根因 | 落法 |
> |---|---|---|
> | 下拉"点开后没办法再点击或者点击空白处关闭" | 实测四条关闭路径**只有一条**真坏：点箭头 ✅ / 点页面空白 ✅ / 点对话框遮罩 ✅ / **再点控件本体 🔴**。ng-select 的 `handleMousedown()` 是 `if (searchable()) { open() } else { toggle() }`，宽屏我们保留 `searchable = true`（长列表要输入过滤）⇒ 永远只 `open()`，对已开的面板是 no-op | 宽屏**只补"关"的那一半**：`pointerdown` 记下"按下时是否已开着"，`pointerup` 里若已开着就 `setTimeout(0)` 再 `close()` |
>
> **两条必须守住的边界**（否则新 bug 比旧 bug 更糟）：
> ① 只在"**按下时已开着**"时才关 —— 否则"点一下打开"会被立刻关掉，表现为**完全打不开**；
> ② 宽屏**绝不能** `preventDefault()` —— 否则输入框落不了光标、选不了文本（这正是 T 段把这段代码
>    限定在窄屏的原因）⇒ 本批是"宽屏也接，但只动关闭"。
>
> **探针教训**：写 `probe-y21-verify.mjs` 时两次差点误判成产品 bug ——
> 窄屏必须用 `touchscreen.tap`（真实触摸），用 `mouse.click` 测不出 pointerdown 定制；
> 而且**每次点击都要重新取坐标**（面板开合会让对话框滚动，复用旧坐标会点到别的控件上，
> 报出"重开失败"这种探针自己造成的假故障）。
>
> | workflow | run | 结论 | 耗时 | 备注 |
> |---|---|---|---|---|
> | `Build Web Vault (patched)`（`version=v2026.8.11`） | **`34959479929`** | ✅ success | **3m48s** | **29 组断言全绿**（本批新增第 29 组 Y 段，只有 `.main.js` 判据） |
> | `Build`（部署） | **`34959845495`** | ✅ success | **4m18s** | 线上 `vw-version.json` = `2026.8.11` |
>
> - 前端源码：fork `shypwd` = **`89ae3d6a19`**（`select.component.{ts,html}` + 版本两处）；本仓库 = **`5011e37`**。
> - 本批**没有跑**离线证伪：一行 CSS/守卫都没动（X 段那次刚跑过），跑一遍结果必然相同。
> - 运行期验收：`probe-y21-verify.mjs` 本地与线上各 **13/0**（桌面 + 移动两档）；
>   回归 `probe-x20-verify.mjs` 桌面 21/0、`probe-w19-verify.mjs` 16/0。
>
