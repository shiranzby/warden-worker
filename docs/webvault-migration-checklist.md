...
| **P5** 迁样式类改动 | A/B/C/E/F/G/H/I/J/J2/J3/K/L/M 共 14 段 / 1248 行 | 🟡 **H + K + M 已落地**（约 210 行） |
| **P6** 迁结构类改动 | TOTP 徽章 / 表头工具行 / 头像卡片 / 验证码页 / 底部标签栏 等 | ⬜ |
...
| F | **移动端（≤768px）布局** | 188 | `warden-filter-open` 等 4 种 | 部分需 JS | ⬜ |
...
| 序 | 内容 | 类型 | 风险 | 依赖 |
|---|---|---|---|---|
| ~~1~~ | ~~H + K + M（留白 / 对话框 / 下拉加固）~~ | CSS | 极低 | — | ✅ 已完成 |
| **2** | **F 段里不依赖 JS 的部分**（保险库页 flex 三栏压缩）+ I 段里不依赖 JS 的部分 | CSS | 低 | — |
| 3 | J8：网格 `minmax(384px→0)` 改 SCSS | SCSS | 低 | — |
| 4 | J7 + J14 + C 段：表头工具行（「选择」/「新增」，批量操作收进 ⋯） | 模板 | 中 | 需 P4 的锚点（已知） |
| 5 | J9 + F 段其余：窄屏筛选抽屉 | 组件 | 中 | 官方 `filter-menu` |
| 6 | J6 + B 段：TOTP 徽章（换官方组件 + 改视觉形态） | 组件 | 中 | 官方 `totp-countdown` |
| 7 | J12 + L 段：行内菜单「文件夹」 | 模板 | 低 | — |
