...
| **P5** 迁样式类改动 | A/B/C/E/F/G/H/I/J/J2/J3/K/L/M 共 14 段 / 1248 行 | 🟡 **H+K+M 已落地**（210 行）、**F 主部已落地**（102 行）、**G 段随 J17 落地**（81 行）、**L 整体淘汰**（J12 改走官方对话框） |
| **P6** 迁结构类改动 | TOTP 徽章 / 表头工具行 / 头像卡片 / 验证码页 / 底部标签栏 等 | 🟡 **J12「文件夹」+ J8 网格 + J17 底部标签栏已落地**；其余 8 项待做 |
...
| 3 | 保险库页内部 flex 三栏（筛选 1/4 + 列表 3/4），窄屏筛选列只剩 **67px** | 🟡 F2 已改纵向堆叠（`8d4292dee2`）；完整形态待 J9（官方 `filter-menu` 抽屉） |
| 4 | 全局侧边导航窄屏被应用自身收成 0px 且**无汉堡按钮** | 🟡 **J17 底部标签栏已落地**（`cbab779ca1`）替代入口；「窄屏隐藏 side-nav」那半边仍**刻意未做**，须与 J9 同批（见 §3 的 ⚠️） |
...
| **4** | J7 + J14 + C 段：表头工具行（「选择」/「新增」，批量操作收进 ⋯） | 模板 | 中 | I 段；I 段需 J17+J9 | ⬅ **下一步**（J17 已就位，但 I 段仍卡 J9，见备注） |
| 5 | J9 + F 段其余：窄屏筛选抽屉 + **「隐藏 side-nav」**（须与 J17 同批，见 §3 的 ⚠️；J17 已完成 → 本条前半的阻塞已解除） | 组件 | 中 | `app-vault-filter` | ⬜ |
...
> 已把 J17 提前完成（`cbab779ca1`）。**下一步仍是 J7，但它还卡在 I 段，
> 而 I 段需要 J9（筛选抽屉）先给出替代入口** —— 所以实际下一条应做 **J9**，
> 用它和已就位的 J17 一起解锁 I 段，再回头做 J7。
