...
| **P5** 迁样式类改动 | A/B/C/E/F/G/H/I/J/J2/J3/K/L/M 共 14 段 / 1248 行 | 🟡 **H+K+M 已落地**（210 行）、**F 主部已落地**（102 行）、**G 段随 J17 落地**（81 行）、**C 段筛选抽屉半边随 J9 落地**（39 行）、**L 整体淘汰**（J12 改走官方对话框） |
| **P6** 迁结构类改动 | TOTP 徽章 / 表头工具行 / 头像卡片 / 验证码页 / 底部标签栏 等 | 🟡 **J12「文件夹」+ J8 网格 + J17 底部标签栏 + J9 筛选抽屉已落地**；其余 7 项待做 |
...
| C | 「名称」表头工具行 + 窄屏筛选抽屉 | 157 | 12 种 | 需 J7/J9 | 🟡 **筛选抽屉半边已落地**（随 J9，样式拆成独立「J9 段」39 行）；「名称」表头工具行待 J7 |
...
| **4** | J7 + J14 + C 段：表头工具行（「选择」/「新增」，批量操作收进 ⋯） | 模板 | 中 | I 段；**I 段两个前置（J17/J9）均已就位** | ⬅ **下一步** |
| 5b | F 段其余：**「隐藏 side-nav」**（J17 就位后阻塞已解除，须与 I 段同批，见 §3 的 ⚠️） | CSS | 中 | J17 | ⬜ |
...
> **至此 I 段的两个前置都已就位，下一步就是 J7**（第 5 行那条「J9 + F 段其余」已拆成 5a/5b：
> 5a 已完成，只剩 5b 的「隐藏 side-nav」，它要和 I 段同批做，否则移动端会失去导航入口）。
